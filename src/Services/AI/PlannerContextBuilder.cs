...
public static class PlannerContextBuilder
{
    ...
    public static string Render(PlannerContext ctx)
    {
        ...
        sb.AppendLine("```");
        sb.AppendLine();
        sb.AppendLine("Schichttausch zwischen zwei Mitarbeitern vermitteln (Admin → erstellt eine");
        sb.AppendLine("Tausch-Anfrage; der Empfänger bekommt eine Benachrichtigung und kann");
        sb.AppendLine("annehmen oder ablehnen):");
        sb.AppendLine("```json");
        sb.AppendLine("{");
        sb.AppendLine("  \"action\": \"swap\",");
        sb.AppendLine("  \"mode\": \"giveAway\" | \"exchange\",");
        sb.AppendLine("  \"fromEntryId\": \"<Schicht des aktuellen Inhabers, abgeben wollen>\",");
        sb.AppendLine("  \"toUserId\": \"<UserId des angefragten Kollegen>\",");
        sb.AppendLine("  \"toEntryId\": \"<nur bei exchange: Gegen-Schicht des Kollegen>\",");
        sb.AppendLine("  \"message\": \"optionale Begründung / Anrede\"");
        sb.AppendLine("}");
        sb.AppendLine("```");
        sb.AppendLine();
